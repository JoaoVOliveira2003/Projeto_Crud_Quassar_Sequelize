...
export const regras: Record<string, FuncaoRegra[]> = {
  ...
  dataDeNascimento: [
    (valor) => !!valor || 'Data é obrigatória.',
  ],
  peso: [
  ...
  cidadeSelecionada: [
    (valor) => (valor !== null && valor !== undefined) || 'Necessário preencher alguma cidade.',
  ],
  email: [
    (valor) => (valor !== null && valor !== undefined) || 'Email obrigatório.',
    (valor) => (typeof valor === 'string' && valor.length >= 3) || 'Email deve ter mais que três caracteres.',
  ],
  senha: [
    (valor) => (valor !== null && valor !== undefined) || 'Senha obrigatória.',
    (valor) => (typeof valor === 'string' && valor.length >= 3) || 'Senha deve ter mais que três caracteres.',
  ],
  novaSenha: [
